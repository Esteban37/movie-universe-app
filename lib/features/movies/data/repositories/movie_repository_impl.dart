...
class MovieRepositoryImpl implements MovieRepository {
  ...
  Future<List<MovieEntity>> getPopular({int page = 1}) async {
    try {
      ...
      throw ServerFailure(e.message ?? 'Failed to fetch popular movies');
    }
  }
  ...
  Future<List<MovieEntity>> getTopRated({int page = 1}) async {
    try {
      ...
      throw ServerFailure(e.message ?? 'Failed to fetch top rated movies');
    }
  }
  ...
  Future<MovieDetailEntity> getMovieDetails(int movieId) async {
    try {
      ...
      throw ServerFailure(e.message ?? 'Failed to fetch movie details');
    }
  }
